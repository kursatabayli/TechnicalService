...
namespace TechnicalService.Portal.OperationalStaff.Pages.ServiceRecords
{
    public partial class UpdateServiceRecord : ComponentBase
    {
        ...
        private async Task LoadData()
        {
            ...
            var personnelsResult = await GetPersonnels.GetAsync(Endpoints.GetPersonnelsByService, ClientTypes.PersonnelAuthClient);
            if (personnelsResult.IsSuccess)
            {
                Personnels = [.. personnelsResult.Data.OrderBy(s => s.Role)];
        ...
        private async Task<IEnumerable<PersonnelDto>> SearchPersonnel(string value, CancellationToken token)
            => await SearchService.SearchAsync(value, Personnels, b => $"{b.Name} {b.Surname}", token);  
        ...
        private string StringFunc(PersonnelDto personnel)
        {
            ...
            return $"{personnel.Name} {personnel.Surname} / {personnel.Role.GetDescription()}";
            else
                return $"{personnel.Name} {personnel.Surname} / {personnel.Role.GetDescription()} ({personnel.PersonnelStatus.GetDescription()})";
        }
